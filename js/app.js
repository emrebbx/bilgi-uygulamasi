(() => {
  ...
  function recallMatch(value,answer) { const typed=normalizeRecall(value),correct=normalizeRecall(answer);if(!typed||!correct)return {accepted:false,exact:false,similarity:0};const exact=typed===correct,similarity=1-(editDistance(typed,correct)/Math.max(typed.length,correct.length));return {accepted:exact||similarity>=.7,exact,similarity}; }
